...
from src.utils import shuffle_boundary_frames
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_compute_temporal_log_prob")
    def compute_temporal_log_prob(self, data: DataProto):
        # when is_lora is True, we use the actor without lora applied to calculate the log_prob
        # which is mostly used for ref log_prob calculation
        assert self._is_actor
        if self._is_offload_param:
        ...
        meta_info = {
            "micro_batch_size": config_source.log_prob_micro_batch_size_per_gpu,
            "max_token_len": config_source.log_prob_max_token_len_per_gpu,
            "use_dynamic_bsz": config_source.log_prob_use_dynamic_bsz,
            "temperature": self.config.rollout.temperature,
        }

        tensors = {key: data.batch[key] for key in ["responses", "input_ids", "attention_mask", "position_ids"]}
        multi_modal_inputs = data.non_tensor_batch["multi_modal_inputs"]

        shuffled_multi_modal_inputs = []
        for idx, multi_modal_data in enumerate(data.non_tensor_batch["multi_modal_data"]):
            ...
            gt_start, gt_end = data.non_tensor_batch["reward_model"][idx]["ground_truth"]
            duration = data.non_tensor_batch["extra_info"][idx]["duration"]
            shuffled_video = shuffle_boundary_frames(videos[0].clone(), video_metadatas[0], gt_start, gt_end, time_radius=3)

            inputs = self.processor(
                text="",
                images=None,
                videos=[shuffled_video],
            ...
            shuffled_multi_modal_inputs.append(inputs)
        
        
        ori_data = DataProto.from_dict(
            tensors=tensors,
            non_tensors={"multi_modal_inputs": multi_modal_inputs},
            meta_info=meta_info,
        )
        shuffled_data = DataProto.from_dict(
            tensors=tensors,
            non_tensors={"multi_modal_inputs": shuffled_multi_modal_inputs},
            meta_info=meta_info,
        )

        # perform recompute log_prob
        with self.ulysses_sharding_manager:
            ori_log_probs, _ = self.actor.compute_log_prob(data=ori_data, calculate_entropy=False)
            shuffled_log_probs, _ = self.actor.compute_log_prob(data=shuffled_data, calculate_entropy=False)
            diff = (ori_log_probs - shuffled_log_probs).cpu()
            response_mask = data.batch['response_mask']
            diff = (diff * response_mask).sum(dim=-1) / response_mask.sum(dim=-1)

            if self.config.temp_diff_thresh == "mean":
                diff_reshaped = diff.view(-1, 8)
                diff_thresh = diff_reshaped.mean(dim=-1, keepdim=True).expand_as(diff_reshaped).reshape(diff.shape)
                diff_thresh = diff_thresh.tolist()
            else:
                diff_thresh = float(self.config.temp_diff_thresh)
                diff_thresh = [diff_thresh] * len(diff)
            
                
            output = DataProto.from_dict(
                non_tensors={
                    "diff_log_prob": diff.tolist(),
                    "diff_log_prob_thresh": diff_thresh
                },
            )
