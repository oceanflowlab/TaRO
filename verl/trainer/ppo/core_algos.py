...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        off_policy_mode = config.get("off_policy_mode", None)
        if off_policy_mode == "AWR":
            new_scores = torch.exp(scores / config.get("awr_beta", 1))
        elif off_policy_mode == "clone":
            new_scores = torch.where(scores > 0,
                     torch.ones_like(scores),
                     torch.zeros_like(scores))
        elif off_policy_mode == "mask":
            new_scores = torch.where(scores > 0,
                     scores,
                     torch.zeros_like(scores))
        elif off_policy_mode == "RAFT":
            max_dict = defaultdict(lambda: -float("inf"))
            for s, idx in zip(scores.tolist(), index):
                if s > max_dict[idx]:
                    max_dict[idx] = s

            new_scores = torch.zeros_like(scores)
            for i, (s, idx) in enumerate(zip(scores, index)):
                if s.item() == max_dict[idx]:
                    new_scores[i] = 1.0
        else:
            new_scores = scores
        
        if off_policy_mask is not None:
            scores = new_scores * off_policy_mask + scores * (1 - off_policy_mask)
...
def compute_policy_loss_vanilla(
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    if off_policy_mask is not None and torch.any(off_policy_mask == 1.0) and not config.enforce_on_policy:
        off_policy_loss = -log_prob * advantages
        off_policy_mask = off_policy_mask.unsqueeze(1)
        pg_losses = off_policy_loss * off_policy_mask + pg_losses * (1 - off_policy_mask)
