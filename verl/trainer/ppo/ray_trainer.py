...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            ...
            config=config,
            off_policy_mask=torch.tensor([_ for _ in data.non_tensor_batch['off_policy']])
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    else:
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                batch.non_tensor_batch["cur_epoch"] = np.array([epoch] * len(batch))
                batch.non_tensor_batch["warmup_epochs"] = np.array([self.config.trainer.get("warmup_epochs", 0)] * len(batch))
                batch.non_tensor_batch["global_steps"] = np.array([self.global_steps] * len(batch))
                batch.non_tensor_batch["steps_per_epoch"] = np.array([len(self.train_dataloader)] * len(batch))
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.actor_rollout_ref.get("temp_diff_thresh", None) is not None:
                        diff_log_prob = self.actor_rollout_wg.compute_temporal_log_prob(batch)
                        for idx in range(len(batch.non_tensor_batch["extra_info"])):
                            batch.non_tensor_batch["extra_info"][idx].update({
                                "diff_log_prob": diff_log_prob.non_tensor_batch["diff_log_prob"][idx],
                                "diff_log_prob_thresh": diff_log_prob.non_tensor_batch["diff_log_prob_thresh"][idx]
                            })
